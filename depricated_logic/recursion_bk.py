def trainRecursion(comparer, dominoes):
    ...
    if (len(matches) == 0) or (len(matches) == 1 and matches[0][0] == matches[0][1]):
        ...
        for match in matches:
            ...
            subresult = trainRecursion(newComparer, newDominoes)
           
            for sub in subresult.values():
                for total,train in sub.items():
                    subtrain.update({keyList[0]:{sum(match) + total : [match] + train}})
                    del keyList[0]
...
if len(multiStarts) > 1:
    for starter in multiStarts:
        ...
        multiTrains.update(subMultiTrain)
